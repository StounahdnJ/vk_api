...
class Controller(object):
    ...
    def __init__(self):
        self.methods = {"new": lambda mes, bool=False: self.__new_followController(mes, bool),
                        "list": lambda mes: self.__all_followController(),
                        "del": lambda mes, bool=False: self.__del_followController(mes, bool)}
        self.command = {'Добавить': {'action': "new", 'mes': "😈Укажите ссылку на пользователя😈"},
                        'Начать': {'action': "new", 'mes': "😈Укажите ссылку на пользователя😈"},
                        'Список': {'action': "list", 'mes': ""},
                        'Удалить': {'action': "del", 'mes': "Напишите номер кого хотите удалить"}}
        self.user = None
    ...
    def update(self):
        ...
        while True:
            for x in user_loc.all_user():  # Получение всех пользователей и проход по ним
                ...
                mes = config.mes['time'].format(year=date.year, day=date.day, month=mounths[date.month - 1],
                ...
                for i in follow['new_friends']:  # Проверка и проход по новым друзьям тех за кем следит
                    for friend in i['friends']:  # Проход именно по тем кто появился
                        mes += config.mes['new friend'].format(id=user.get_name(i['id'])['name'],
                                                               friend=user.get_name(friend)['name']) + "\n"
                        col += 1
                    user.update_follow(i['id'])

                for i in follow['del_friends']:  # Проверка и проход по удаленным друзьям тех за кем следит
                    for friend in i['friends']:  # Проход именно по тем кто появился
                        mes += config.mes['del friend'].format(id=user.get_name(i['id'])['name'],
                                                               friend=user.get_name(friend)['name']) + "\n"
                        col += 1
                    user.update_follow(i['id'])
                ...
                mes += "\n" + config.mes['dop']
                if col > 0:
                    if not user.message(mes):
                        follows = user.get_follow()
                        if follows['code']:
                            for i in follows['items']:
                                user.del_follow(follow_id=i['id'])
                            self.__log("Удалил список у {id}".format(id=x))

            self.__log('Сделал обновление')
